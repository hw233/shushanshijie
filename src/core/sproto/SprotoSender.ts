	...
	export class SprotoSender {
		...
		public static Pack(tag: number, rpcReq: SprotoTypeBase = null, rpcRspHandler: Function = null, thisObj: any = null): egret.ByteArray {
			if (rpcRspHandler != null) {
				...
				console.log("Sproto-Pack-session",session);
				SprotoSender.m_RpcRspHandlerDict[session] = thisObj && rpcRspHandler.bind(thisObj) || rpcRspHandler;
				console.log("SprotoSender.m_Protocol.Get",SprotoSender.m_Protocol.Get(tag).ResponseType);
		...
		public static SendData(rpc: SprotoTypeBase, session, tag: number): egret.ByteArray {
			...
			if (rpc != null) {
				console.log(rpc)
				console.log(tag)
				console.log(SprotoSender.m_Protocol.Get(tag).RequestType)
				console.log(Sproto.ALL_DICT[SprotoSender.m_Protocol.Get(tag).RequestType])
		...
		public static HandlerSession(session: number, data: Uint8Array, offset: number): void {
			console.warn("SprotoSender.Handler session => " , session);
			...
			if (responseType && responseFunc) {
				...
				console.warn("SprotoSender.Handler responseType => " , responseType);
				console.warn("SprotoSender.Handler responseFunc => " , responseFunc);
			}
