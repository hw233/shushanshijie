	...
	public initThumbnail(w, h, fName) { 
		...
		if (this.m_MapUrl) {
			...
			for (let i = this.m_ThumbList.length - 1; i >= 3; --i) {
				RES.destroyRes(this.m_ThumbList[i])
			}
			this.m_MapUrl = null
		}
		let url = ResDataPath.GetMapPreviewPath(this.mapName)
		this.m_MapUrl = url
		let resInfo: RES.ResourceInfo = {url: url, type:RES.ResourceItem.TYPE_IMAGE, root: "", name: "pk_preview_jpg"};
		let host: RES.ProcessHost = {state:null, resourceConfig: RES["config"], load:null, unload:null, save:null, get:null, remove:null}
		RES.processor.ImageProcessor.onLoadStart(host, resInfo).then((img)=>{
			this._Finish(img);
		});
		
	}

	private _Finish(resItem): void {
		if (resItem) {
			this.thumbnail.source = resItem;
			this.onThumbnailComplete()
		} else {
			console.error("MapViewBg._Finish resItem.name != this.m_MapUrl", resItem, this.m_MapUrl)
		}
	}      

	public updateHDMap(point: any, force: boolean = false) { // 点击地图行走并获取地图
		var IMG_SIZE = GameMap.chunkw //data.chunkw || 512  (GameMap.ts中 168行)
		...
		if (force || Math.abs(this.lastUpdateX - point.x) > 60 || Math.abs(this.lastUpdateY - point.y) > 60 || this.lastUpdateX == 0) {
			...
			if (!this.isThumbnailComplete)
				return;
			var ww = GameGlobal.StageUtils.GetWidth(); //获取游戏的宽
			// console.log("游戏宽度"+ww) 960
			var hh = GameGlobal.StageUtils.GetHeight();  //获取游戏的高
			...
			for (var i = imgX; i < xCount; i++) {
				for (var j = imgY; j < yCount; j++) {
					...
					// refresh = true
					this._imageList[index] = this.GetImage(i, j, ResDataPath.GetMapPath(this.mapName, i, j)) //获取地图资源
				}
			}
