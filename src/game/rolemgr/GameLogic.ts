...
class GameLogic extends BaseSystem {
    ...
    public doExp(rsp: Sproto.exp_change_request) {
        ...
        let newExp = rsp.exp;
        model.exp = newExp;
        let uid = Main.getInstance().UserName;
        // 合区的话，这里的uid要加上尾缀
        if(Main.getInstance().mConnectServerData.id <= window["__CONFIG__"]["__HE_SERVER__"]){
            uid = uid + "_"+Main.getInstance().mConnectServerData.id;
        }
        if (oldExp != newExp)
        ...
        if (oldLevel < model.level) {
            var roledatas = {
                ...
                uid:uid,
                serverId:Main.getInstance().mConnectServerData.id,
                serverName:Main.getInstance().mConnectServerData.id + "区",
                roleId:GameGlobal.actorModel.actorID,
                roleName:GameGlobal.actorModel.name,
                level:model.level,
                sign:rsp.sign,
            };
            console.log('上传角色信息',roledatas)
            if(window["TYPE_NUM"] == 1){
                platform.upgradeRole(roledatas).then(function(res){
                    console.log(res);
                })
            }else{
                window["__REC__"].upgradeRole(roledatas, (res) => {
                    console.log(res);
                });
            }
            this.levelEffect(model.level);
        }
        if (rsp.upexp > 0) {
    ...
    public doFirstRegister(bytes) {
        console.log("doFirstRegister-第一次登陆")
    ...
    /** 播放升级特效 */
    public levelEffect(level) {
        if(typeof(window['android'])!="undefined" && typeof(window['android'].anUploadRole) !="undefined"){
            //供于android使用
            let para = {lv:level};
            window['android'].anUploadRole(JSON.stringify(para));
        }
