class RoleMgr extends BaseSystem {
	...
	public doCheckAccount(data: Sproto.checkAccount_response) {
		...
		if (result == 0) {
			...
			//被顶号
			if (result == -1) {
				console.log("服务器正在维护中，请稍后");
			}else {
				console.log(RoleMgr.LONGIN_ERROR_CODE[result]);
				if (result == 3) {
					window["connectError"]();
				}else if (result == 1) {
					// 账号信息已过期，请重新登录
	...
	public doRoleList(data: Sproto.QueryList_response) {
		...
		if (RoleMgr.GetRoleState(data) != 0) {
			...
			let createRoleData = Main.getInstance().mCreateRoleData
	...
	public doEnterGame(data: Sproto.EnterGame_response) {
		switch (data.result) {
			...
			default:
				if (data.result == 4) {
					console.log("帐号被封停，请联系客服QQ：" + (data.QQ || ""));
				} else {
					console.log("进入游戏错误码:" + data.result);
				}
				break;
		}
	};
	...
	public sendCreateRole(actorname, sex, job) {
		...
		RoleMgr.actorname = CreateActor.actorname = actorname;
		RoleMgr.sex = CreateActor.sex = sex;
		RoleMgr.job = CreateActor.job = job;
		CreateActor.icon = 0
		CreateActor.pf = ""
	...
	public doCreateRole(data: Sproto.CreateActor_response) {
		if (data.result == 0) {
			...
			let roledata = {
				...
				roleId:data.actorid,
				roleName:this.m_TempName,
				sign:data.sign,
			}
			console.log("this.roledata",JSON.stringify(roledata))
			 if(window["TYPE_NUM"] == 1){
                platform.CreateRole(roledata).then(function(res){
                    console.log(res);
                })
            }else{
				window["__REC__"].CreateRole(roledata, (res) => {
					console.log(res);
				});
			}
		}
	...
	public showErrorTips(result: number) {
		...
		console.log(RoleMgr.Create_Actor_Error[result])
	};
