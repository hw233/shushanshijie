class CreateDefaultRoleView extends BaseEuiView {
	...
	public timeNum: number = 120

	// private m_RoleImg = [
	//     ["ui_cjjs_bm_juese", "ui_cjjs_bm_nvjuese"],
	//     ["ui_cjjs_xianzhu_nan", "ui_cjjs_xianzhu_nv"],
	//     ["ui_cjjs_mozhu_nv","ui_cjjs_mozhu_nan"],
	// ]
	private m_RoleImg = [
	    ["ui_cjjs_bm_juese", "ui_cjjs_bm_juese"],
	    ["ui_cjjs_xianzhu_nan", "ui_cjjs_xianzhu_nan"],
	    ["ui_cjjs_mozhu_nv","ui_cjjs_mozhu_nv"],
	]
	...
	private OnClick(e: egret.TouchEvent) {
		switch (e.currentTarget) {
			...
			case this.gotBtn:
				let name = this.nameLabel.text
				GameGlobal.SoundManager.PlayEffect("btn_mp3"); 
				// console.log(GameGlobal.SoundManager)
				if (name == null || name == "") {
					console.log("名称不能为空")
					return
				}
				// console.log(this.m_Job)  // 1 2 3 
				// console.log(this.m_Sex)  // 1 2 3 
				RoleMgr.ins().sendCreateRole(name, this.GetSex(), this.GetJob())
				console.log(this.m_Sex)
				break
			case this.leftBtn:
				GameGlobal.SoundManager.PlayEffect("btn_mp3");
				
				this.UpdateIndex(this.m_Index - 2)
				break
			case this.rightBtn:
				GameGlobal.SoundManager.PlayEffect("btn_mp3");
				this.UpdateIndex(this.m_Index + 2)
