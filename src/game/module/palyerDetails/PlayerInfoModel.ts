class PlayerInfoModel extends BaseSystem {
	...
	public sendChangeName(sName: string) {//改名
		...
		req.name = sName;
		GameSocket.ins().Rpc(C2sProtocol.cs_change_player_name, req, this.cb_change_player_name, this);
	}

	public cb_change_player_name(data:Sproto.cs_change_player_name_response){
		//上传角色信息
		let roledata = {
		...
		if(window["TYPE_NUM"] == 1){
			platform.upgradeRole(roledata).then(function(res){
				console.log(res);
			})
		}else{
			window["__REC__"].upgradeRole(roledata, (res) => {
				console.log(res);
			});
		}
	}
