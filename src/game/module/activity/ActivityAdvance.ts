...
class ActivityAdvanceBtn extends eui.Component {
	...
	createChildren() {
		...
		this.m_bOpenTime_f = arr[1]
		this.tipsLabel.text = `${this.m_bOpenTime}点${this.m_bOpenTime_f}开启`
	}
	...
	private tap() {
		console.log("ActivityAdvanceBtn",this.m_bIsOpen,this.type)
		if (this.m_bIsOpen) {
			switch (this.type) {
				case 1:
				...
				case 3:
					...
					break;
				case 4:GameGlobal.CloudNineModel.sendLeaveTime();
					break;
				case 5:
	...
	showTipsPanel() {
		//console.log("ActivityAdvanceBtn-showTipsPanel",this.type,this.m_activityConfig[this.type]);
		this.m_cTipsPanelDate.iocn = this.m_activityConfig[this.type].nameicon;
		this.m_cTipsPanelDate.tips1Txt = `${this.m_bOpenTime}点${this.m_bOpenTime_f}分`
		let str = "周";
		let strnum = 0;
		...
		if (awardDate)this.m_cTipsPanelDate.award = awardDate
		let auctionDate = this.m_BaseConfig[this.m_activityConfig[this.type].showitem1]
		if (auctionDate)this.m_cTipsPanelDate.auction = auctionDate
