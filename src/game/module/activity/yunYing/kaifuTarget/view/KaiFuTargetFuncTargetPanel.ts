class KaiFuTargetFuncTargetPanel extends BaseView {
	...
	private ShowDesc(cfgObj) {
		...
		switch (type) {
			case "arenarank":
				arr = ["比武场排名：", GameGlobal.Arena.getMyRank()]
			break;
			case "wildgeese":
				arr = ["锁妖塔层数：", GameGlobal.UserFb.GetLinglongLayer()]
			break;
			case "chapter":
				arr = ["关卡关数：", (GameGlobal.UserFb.guanqiaID || 0)]
			break;
			case "heaven":
				arr = ["紫禁之巅：", (GameGlobal.UserFb.tFbTiantingServerData ? GameGlobal.UserFb.tFbTiantingServerData.todayLayer : 0)]
			break;
			case "treasuremap":
				arr = ["连城宝藏层数：",Math.max(FbCbtPanel.GetLastNum() - 1, 1)]
			break;
			case "petcount":
				arr = ["伙伴数量：", GameGlobal.PetModel.GetActiveCount()]
			break;
			// case "enhancelevel":
			// break;
			// case "refinelevel":
