class ActivityKaiFuModel extends BaseSystem {
	...
	public constructor() {
		...
		this.regNetMsg(S2cProtocol.sc_zj_activity_update_info, this._DoActivityUpdate_zj)
		this.regNetMsg(S2cProtocol.sc_ch_activity_update_info, this._DoActivityUpdate_ch)
	...
	//坐骑礼包
	private _DoActivityUpdate_zj(rsp:Sproto.sc_zj_activity_update_info_request){
		let index = rsp.index;
		let res = rsp.res;
		if(res == 1){
			GameGlobal.RechargeModel.finish[80] = true;
		}
		GameGlobal.MessageCenter.dispatch(MessageDef.ACTIVITY_ZUOJI_GIFT, index, res);
	}
	
	//称号礼包
	private _DoActivityUpdate_ch(rsp:Sproto.sc_ch_activity_update_info_request){
		let index = rsp.index;
		let res = rsp.res;
		if(res == 1){
			if(index == 81){
				GameGlobal.RechargeModel.finish[81] = true;
			}else{
				GameGlobal.RechargeModel.finish[82] = true;
			}
		}
		GameGlobal.MessageCenter.dispatch(MessageDef.ACTIVITY_TITLE_GIFT, index, res);
	}
