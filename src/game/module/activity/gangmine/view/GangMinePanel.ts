class GangMinePanel extends BaseEuiView {
    ...
    initUI() {
        ...
        this.commonWindowBg.SetTitle('灵石争夺');
    };
    ...
    OnOpen(...args: any[]) {
        ...
        this.AddClick(this.btnHelp, this.onClickHandler);
        let i = 0, len = GangMineModel.MINE_MAX_NUM;
        for (i = 0; i < len; i++) {
            let kuang: GangMineKuangItem = this['kuang' + i];
    ...
    private onClickHandler(e: egret.TouchEvent) {
        ...
        if (egret.is(e.currentTarget, 'GangMineKuangItem')) {
            ...
            switch (kuang.state) {
                case 1:
                    WarnWin.show('该灵山有一群小妖占领着，是否赶跑妖怪开始占领采矿？', this.sureHandler, this);
                    break;
                case 2:
                ...
                case 4:
                    ...
                    break;
                case 5:
                    if (this.model.myInfo.mineId == this.mineId) {//表示我自己精灵的
                        this.model.sendGangMineDeatial(this.mineId);
                        ViewManager.ins().open(GangMineTeamGuardPanel, this.mineId)
                    }
                    else GameGlobal.UserTips.showTips('该矿脉遭受到别帮攻击，暂时不能加入精灵')
                    break;
            }
            return;
        }
        ...
        else if (e.target == this.btnHelp){
            console.log(GlobalConfig.ins().HelpInfoConfig)
            ViewManager.ins().open(ActivityDescPanel, 47, "规则说明")
        }
    }
    ...
    private showOtherData() {
        this.titleTxt.text = this.curpage + '号灵山';
...
class GangMineKuangItem extends eui.Component {
    ...
    /**占领状态 1 小怪， 2 玩家占领， 3=怪物精灵战斗中， 4=玩家精灵战斗中， 5=玩家精灵战斗过(不可加入)*/
