class PetXilianPanel extends BaseEuiView {
	...
	public constructor() {
		super()
		this.skinName = "PetXilianSkillSkin" //外观名称
		this.commonWindowBg.SetTitle("武功洗练") //设置当前窗口的title名称
		//添加点击事件
		...
		for (let i = 0; i < this.group01.numChildren; i++) {//左
			let item = this.group01.getChildAt(i) as PetXilianItem //？
			this.SetBg(item, i) //设置item背景外观（间隔）
			this.mItemLeft.push(item) //循环遍历每个item并添加到mItemLeft中
			item.lockImg.name = i + "" //赋值并记录每一个lockImg
			this._AddClick(item.lockImg, this._OnLockClick) //添加点击事件

			item.skillGroup.name = i.toString() //设置item中skillGroup的name属性
			this._AddClick(item.skillGroup, this._OnSkillIconClick) //添加点击
		}
		for (let i = 0; i < this.group02.numChildren; i++) {//右
	...
	private _OnClick(e: egret.TouchEvent) {
		switch (e.currentTarget) {
			case this.btn01: //普通洗练
				GameGlobal.SoundManager.PlayEffect("btn_mp3");
				this.SendXilian(0)
				break
			case this.btn02: //高级洗练
				GameGlobal.SoundManager.PlayEffect("btn_mp3");
				this.SendXilian(1)
				break
			case this.btnchage: //更换
				GameGlobal.SoundManager.PlayEffect("btn_mp3");
				this.SendSkillXilian()
				break
			case this.btnInfo: //帮助
				GameGlobal.SoundManager.PlayEffect("btn_mp3");
	...
	//普通洗练或者高级洗练
	private SendXilian(type: number) { //普通0高级1
		let baseConfig = this.GetBaseConfig()
	...
	private _OnLockClick(e: egret.TouchEvent) {
		GameGlobal.SoundManager.PlayEffect("btn_mp3");
		let index = parseInt(e.currentTarget.name) //i
		let item = this.mItemLeft[index]
		if (this._IsLastOne() && !item.lock) {
			UserTips.ins().showTips("武功不可全部锁定")
			return
		}
		item.lock = !item.lock
