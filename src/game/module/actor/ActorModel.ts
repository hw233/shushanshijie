class ActorModel {
	...
	public static IsGM(): boolean {
		return Main.getInstance().GmLevel == 100
	}
	...
	public static IsSimpleGM(): boolean {
		...
		return Main.getInstance().GmLevel >= 1
	}
	...
	public parser(data: Sproto.sc_actor_base_request) {
		...
		if (Main.getInstance().mConnectServerData) {
			Main.getInstance().mConnectServerData.id = data.serverid
		}
		...
		if (window["_QuickSDK_"]) {
			let roleInfo:any = {};	
			roleInfo.isCreateRole = data.level <= 1 ? true : false;	
			roleInfo.roleCreateTime = 1490598150;
			
			roleInfo.uid = Main.getInstance().UserName;
			
			roleInfo.serverId =  Main.getInstance().mConnectServerData.id;
			
			roleInfo.serverName = Main.getInstance().mConnectServerData.sname + "";
			
			roleInfo.userRoleName = data.actorname;
			
			roleInfo.userRoleId = data.actorid;
			
			roleInfo.userRoleBalance = 1000;
			
			roleInfo.vipLevel = 1;
			
			roleInfo.userRoleLevel =  data.level;
			
			roleInfo.partyId = 1;
			
			roleInfo.partyName = '';
			
			roleInfo.gameRoleGender = data.sex;
			...
			roleInfo.professionId = data.job;
			
			roleInfo.profession = data.job;
		...
		if (window["OPEN_DATA"]) {
			TDGA.Account({
				accountId : data.actorid,
				level : data.level,
				gameServer : Main.getInstance().mConnectServerData.name,
				accountType : 1,
				age : 24,
				accountName : data.actorname,
				gender :  data.sex
			});
		}
	}
